const STORAGE_KEY = 'baseballai_anthropic_key';
const VOICE_KEY = 'baseballai_voice_name';
const MUTE_KEY = 'baseballai_muted';
const HISTORY_KEY = 'baseballai_history';
const MODEL = 'claude-sonnet-4-6';

const SYS_BASEBALL = `You are BASEBALL.AI, a sharp, no-nonsense MLB analyst. You have direct access to the live MLB Stats API via tools. ALWAYS use the tools for stats, scores, rosters, and player data — NEVER invent or guess numbers.

WHEN TO USE WHICH TOOL:
- A player name, "tell me about X", "who is X", "give me an overview of X", or "show me X" → player_dossier. The UI renders the full personnel-file card (bio + season + career + full breakdown). Add only 2–3 sentences of insight (notable trait, current form, recent storyline). DO NOT re-list bio fields.
- Specific stat lookups ("what's his AVG?") → player_stats.
- A team name, "tell me about the Yankees", "show me the Dodgers" → team_dossier. UI handles bio, record, roster.
- Any "leaders", "leaderboard", "top N", "best in X", "who leads", "X leaders for [league/division]" → show_leaderboard. UI opens a ranked panel. Just briefly acknowledge — DO NOT list players in your text. Supports league (AL/NL) and division ("NL West", etc.) filters. WAR is NOT available from the MLB API; for WAR/best-overall queries, say so directly and offer OPS (hitters) or ERA (pitchers).
- "Scores", "scoreboard", "today's games" → show_scoreboard.

Voice: confident analyst, dry, terse. Skip filler. Don't address the user with a title — just answer.`;
const SYS_GENERAL = SYS_BASEBALL;
...
function appendTurn(role, text) {
  ...
  const label = role === 'user' ? 'YOU' : 'BB.AI';
...
els.saveKey.addEventListener('click', () => {
  ...
  if (!key.startsWith('sk-ant-')) {
    alert('That does not appear to be an Anthropic API key. They begin with "sk-ant-".');
    return;
  }
...
els.clearChat.addEventListener('click', () => {
  state.history = [];
  saveHistory();
  els.transcript.innerHTML = '<div class="placeholder">Player name, team, leaders, scores — ask anything.</div>';
  renderSuggestions();
});
...
function buildGamesHTML(games) {
  if (!games.length) return '<div class="games-loading">No games today.</div>';
  games.sort((a, b) => {
...
function openLeaderboard(opts) {
  ...
  if (!opts.leaders.length) {
    rowsHtml = '<div class="games-loading">No data on file.</div>';
  } else {
...
async function fetchGames() {
  ...
  try {
    ...
    if (!games.length) {
      body.innerHTML = '<div class="games-loading">No games today.</div>';
    } else {
      games.sort((a, b) => {
        const rank = (g) => g.state === 'Live' ? 0 : g.state === 'Preview' ? 1 : 2;
        return rank(a) - rank(b);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (test) {
    test.addEventListener('click', () => {
      ...
      const u = new SpeechSynthesisUtterance('Voice ready. Live MLB feed online.');
      if (voice) u.voice = voice;
...
const mlb = {
  ...
  async show_leaderboard({ stat, group, season, league, division, limit }) {
    ...
    if (!cat || !cat.leaders?.length) {
      return { error: `"${stat}" is not a recognised leaderboard category on the official MLB Stats API. Try OPS, AVG, OBP, SLG, HR, RBI, hits, stolen bases (hitting) or ERA, WHIP, wins, saves, strikeouts, K/9 (pitching).` };
    }
    ...
    if (!leaders.length) {
      return { error: `No qualified players returned for ${usedStat} in ${scopeLabel}.` };
    }
...
const baseballTools = [
  { name: 'team_dossier',
    description: 'Open a floating, draggable FRANCHISE FILE panel for an MLB team: logo, division, venue, record (W-L, pct, division rank, GB, streak, run diff, last 10, home/away splits), full team hitting + pitching breakdowns, and the active roster. The user can double-click any roster player to open their dossier. ALWAYS use this when asked for an overview of a team ("show me the Dodgers", "tell me about the Yankees", just a team name). Do NOT re-list bio fields in your text — just acknowledge briefly.',
    input_schema: { type: 'object', properties: {
      name: { type: 'string', description: 'Team name (e.g. "Dodgers", "Yankees", "NYY")' },
    }, required: ['name'] } },
  { name: 'show_scoreboard',
    description: 'Open a floating, draggable LIVE SCOREBOARD panel that shows every MLB game today with current scores and inning. The UI handles the data — just call the tool, then briefly acknowledge. Use whenever the user asks to see, open, or pull up scores, games, or the scoreboard.',
    input_schema: { type: 'object', properties: {} } },
  { name: 'player_dossier',
    description: 'Build a comprehensive personnel-file dossier for a player: bio (height, weight, age, bats/throws, debut, origin) plus current-season and career stats for hitting/pitching as applicable. Use this whenever the user asks for an overview of a player ("tell me about X", "who is X", "show me X", or just a bare name). Returns a "dossier" object — the UI renders it as a personnel file automatically, so do not repeat the bio fields in your response. Add a short analytical summary (2-3 sentences) about the player\'s notable trait or current form.',
    input_schema: { type: 'object', properties: {
  ...
  { name: 'show_leaderboard',
    description: 'Open a floating, draggable LEADERBOARD panel showing top players for a stat (rank, headshot, name, team, value). The user can double-click any row to open that player\'s full dossier. ALWAYS use this for any "leaders" / "leaderboard" / "best in X" question — never list leaders in prose. Supports optional league (AL/NL) and division ("NL West", "AL East") filters. After calling, briefly acknowledge. IMPORTANT: WAR is NOT available via MLB Stats API — for WAR / "best overall" queries, say so and offer OPS (hitters) or ERA (pitchers).',
    input_schema: { type: 'object', properties: {
...
function renderHistoryTables(statsArr) {
  ...
  if (!any) return '<div class="hp-err">No archive data on file for this player.</div>';
  return html;
}
...
async function showHistory(d) {
  ...
  panel.innerHTML = `
    ...
    <div class="hp-body"><div class="hp-loading">Compiling archive...</div></div>
  `;
  document.body.appendChild(panel);
...
async function callJarvis(userText) {
  ...
  console.log(`[baseballai] sending text="${userText}"`);

  try {
    await runWithTools(apiKey, jarvisTurn, jarvisText);
  } catch (err) {
    console.error(err);
    jarvisText.textContent = 'Connection to the MLB feed is unstable. Try again.';
...
async function handleApiError(res, jarvisText) {
  ...
  if (res.status === 401) {
    jarvisText.textContent = 'Invalid Anthropic API key.';
    speak('Invalid API key.');
    state.history.pop();
    setStatus('AUTH ERROR', 'error');
    openSettings();
  } else {
    jarvisText.textContent = `Error — ${errMsg}`;
...
function send() {
  ...
  if (lb) {
    if (lb.unsupported) {
      quickReply(text, `${lb.unsupported} is a Sabermetric stat calculated externally by Fangraphs and Baseball Reference — the official MLB Stats API does not provide it. Try OPS, AVG, OBP, SLG, HR, RBI, hits, or stolen bases for hitters; ERA, WHIP, wins, saves, strikeouts, or K/9 for pitchers.`);
      return;
    }
  ...
  if (tm) {
    quickReply(text, `Bringing up the ${tm.name}.`);
